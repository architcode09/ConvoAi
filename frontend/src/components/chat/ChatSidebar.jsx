...
import { setActiveConversationId, setSearchQuery, setSidebarTab } from "../../store/chatSlice";

function mapUserForList(user, onlineUsers) {
  const isOnline = onlineUsers.some((onlineUserId) => String(onlineUserId) === String(user._id));
  return {
...
function ChatSidebar() {
  ...
  const sidebarTab = useAppSelector((state) => state.chat.sidebarTab);
  ...
  return (
    ...
    >
      ...
      <Tabs
        ...
        onSelectionChange={(key) => dispatch(setSidebarTab(String(key)))}
        variant="secondary"
          ...
          {filteredUsers.length === 0 ? (
            ...
            filteredUsers.map((user) => (
              <ConversationRow
                ...
                onSelect={() => dispatch(setActiveConversationId(user.conversationId))}
              />
            ))
          )}
        </Tabs.Panel>
